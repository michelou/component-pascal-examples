...
if defined _SEARCH_JROOT (
    if exist "%JROOT%\jars\" (
        call :search "%JROOT%\jars" 1
        if not !_EXITCODE!==0 goto end
    )
)
goto end
...
if "%__ARG:~0,1%"=="-" (
    ...
    ) else if "%__ARG%"=="-jroot" ( set _SEARCH_JROOT=1
...
:args_done
if %_DEBUG%==1 (
    echo %_DEBUG_LABEL% Options   : _SEARCH_JAVA=%_SEARCH_JAVA% _SEARCH_JROOT=%_SEARCH_JROOT% _VERBOSE=%_VERBOSE% 1>&2
