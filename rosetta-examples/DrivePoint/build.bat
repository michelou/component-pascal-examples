...
rem copy /y "%_ROOT_DIR%symfiles\Strings.cps" "%_CLASSES_DIR%"

if %_DEBUG%==1 ( set __GPCP_OPTS=-verbose
) else if %_VERBOSE%==1 ( set __GPCP_OPTS=-quiet
) else ( set __GPCP_OPTS=-quiet -nowarn -list-
)
set "__CPSYM="%_ROOT_DIR%symfiles;%JRoot%\symfiles;%JRoot%\symfiles\JvmSystem"
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% __CPSYM=!__CPSYM:%JRoot%=%%JRoot%%! 1>&2
)
@rem default is -asm7 for class files generation
set __GPCP_OPTS=%__GPCP_OPTS% -asm8 -cpsym:__CPSYM

@rem source file paths relative to directory 'target\classes\'
set __SOURCE_FILES=
set __N=0
for /f "delims=" %%f in ('dir /b "%_SOURCE_MAIN_DIR%\P*.cp" "%_SOURCE_MAIN_DIR%\D*.cp" 2^>NUL') do (
...
rem call "%_GPCP_JVM_CMD%" %__GPCP_OPTS% %__SOURCE_FILES%
call "%_GPCP_JVM_CMD%" %__GPCP_OPTS% "..\..\!_SOURCE_MAIN_DIR:%_ROOT_DIR%=!\Point.cp"
if not %ERRORLEVEL%==0 (
    popd
...
call "%_GPCP_JVM_CMD%" %__GPCP_OPTS% "..\..\!_SOURCE_MAIN_DIR:%_ROOT_DIR%=!\DrivePoint.cp"
popd
goto :eof
